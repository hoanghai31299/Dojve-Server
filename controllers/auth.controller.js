...
exports.signin = async(req, res, next) => {
    try {
        ...
        const token = await jwt.sign({ user: { _id: user._id } },
            process.env.JWT_TOKEN_SECRET, {
                expiresIn: "1d",
            }
        );
        const refreshToken = await jwt.sign({ user: { _id: user._id } },
            process.env.JWT_REFRESH_TOKEN_SECRET, { expiresIn: "30d" }
        );
...
exports.signinByCookie = async(req, res, next) => {
    try {
        ...
        if (decoded) {
            ...
            let newToken = await jwt.sign({ user: { _id: newU._id } },
                process.env.JWT_TOKEN_SECRET, {
                    expiresIn: "1d",
                }
            );
            ...
            if (user) {
                ...
                const token = await jwt.sign({ user: { _id: newU._id } },
                    process.env.JWT_TOKEN_SECRET, {
                        expiresIn: "1d",
                    }
                );
